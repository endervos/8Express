...
module.exports = {
  async up(q, S) {
    ...
    await q.addConstraint("PostReaction", {
      fields: ["admin_id"], type: "foreign key", name: "fk_postreaction_admin",
      references: { table: "Admin", field: "id" }, onDelete: "CASCADE", onUpdate: "CASCADE"
    });
    await q.addConstraint("PostReaction", {
      fields: ["post_id", "user_id"], type: "unique", name: "uq_post_user"
    });
    await q.addConstraint("PostReaction", {
      fields: ["post_id", "admin_id"], type: "unique", name: "uq_post_admin"
    });
    await q.addIndex("PostReaction", ["post_id", "reaction_id"]);
  },
  async down(q) {
    await q.dropTable("PostReaction");
  }
};